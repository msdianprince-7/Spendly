...
## Files to create
None. Query helpers were added directly to `database/db.py` (not a separate
`database/queries.py`), per CLAUDE.md's rule that DB logic lives only in
`database/db.py`:
  - `get_user_by_id(user_id)` → dict with `name`, `email`, `member_since`
  - `get_expense_stats(user_id)` → dict with `total_spent`, `transaction_count`, `top_category`
  - `get_expenses_by_user(user_id)` → list of dicts, each with `date`, `description`, `category`, `amount`
  - `get_category_breakdown(user_id)` → list of dicts, each with `name`, `total`, `percent` (percentage of total, rounded to nearest int)
...
- `total_spent` matches sum of all seed expenses (798.54)
...
- [x] Total spent displayed on the profile page equals ₹798.54
- [x] Transaction count displayed is 8
- [x] Top category displayed is "Bills"
